...
import { Files, FilePlus, Settings, Layout, Sun, Moon } from "lucide-react";
...
function App() {
  ...
  return (
    <div className={`flex flex-col min-h-screen ${theme === "dark" ? "bg-slate-950" : "bg-slate-50"} transition-colors duration-500`}>
      {/* Navigation Header */}
      <nav className={`sticky top-0 z-50 backdrop-blur-md border-b ${theme === "dark" ? "bg-slate-900/80 border-slate-800" : "bg-white/80 border-slate-200"} px-6 py-4`}>
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          <Link to="/" className="flex items-center gap-3 group transition-all">
            ...
            <span className={`font-heading font-bold text-2xl tracking-tight ${theme === "dark" ? "text-white" : "text-slate-900"}`}>
      ...
      <footer className={`mt-auto border-t py-10 px-6 ${theme === "dark" ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2 opacity-50 grayscale contrast-125">
            ...
            <span className={`font-heading font-bold text-lg tracking-tight ${theme === "dark" ? "text-white" : "text-slate-700"}`}>
              DocSignApp
            </span>
          </div>
          <p className="text-slate-500 text-sm font-medium">&copy; 2026 DocSignApp. All rights reserved.</p>
          <div className="flex gap-8 text-slate-500 text-sm font-medium">
            ...
            <a href="#" className="hover:text-primary-500 transition-colors">Help</a>
          </div>
        </div>
      </footer>
    </div>
